...
const CourseDetail = () => {
  ...
  const {allCourses,calculateRating,calculateChapterTime,currency,calculateCourseDuration,calculateNoOfLectures,backendUrl,userData,getToken}=useContext(AppContext)
    ...
    <div className='flex md:flex-row flex-col-reverse  gap-10 relative  items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        {/* review and rating */}
         ...
         <div className='flex' >
         {[...Array(5)].map((_,i)=>(<img key={i} src={i<Math.floor(calculateRating(courseData))?assets.star:assets.star_blank} className='w-3.5 h-3.5' alt='rating'/>)
       )}
       </div>
         ...
         <div className={`overflow-hidden translate-all duration-300  ${openSection[index]?'max-h-96':'max-h-0'}`}>
           <ul className='list-disc  md:pl-10 pl-4 pr-4  py-2 text-gray-600 border-t  border-gray-300'>{chapter.chapterContent.map((lecture,i)=>(
             <li key={i} className='flex items-start gap-2 py-1'>
               ...
               <div className='flex gap-2'>{lecture.isPreviewFree && <p onClick={()=>setPlayerData({vedioId:lecture.lectureUrl.split('/').pop()})}className='text-blue-500 cursor-pointer'>Preview</p>}
