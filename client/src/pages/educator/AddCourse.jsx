...
const handleSubmit= async (e)=>{
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-baseline md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={e=>setCourseTitle(e.target.value)} value={courseTitle} type="text"required className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500'placeholder='Type here' name="" id="" />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input type="number" placeholder='0' onChange={e=>setCoursePrice(e.target.value)} value={coursePrice} required className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500'/>
          </div>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img src={assets.dropdown_icon} width={14} className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`}  onClick={() => handleChapter('toggle', chapter.chapterId)} alt="" />
                ...
                <div className='p-4'>{
                  chapter.chapterContent.map((lecture,lectureIndex)=>(
                    <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                      <span>{lectureIndex+1}{lecture.lectureTitle} - {lecture.lectureDuration}mins - <a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a>- {lecture.isPreviewFree? 'Free Previw':'Paid'}</span>
              ...
              <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
                <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                  ...
                  <div className='mb-2'>
                    ...
                    <p>Is Preview Free?</p>
                    <input type="checkbox" className='  scale-125' value={lectureDetails.isPreviewFree} onChange={e=>setLectureDetails({...lectureDetails, isPreviewFree:e.target.value})} />
                  </div>
                  <button type='button' className='w-full bg-blue-500 text-white py-2 px-4 rounded'onClick={addLecture}>Add</button>
                  <img onClick={()=>setShowPopup(false)} src={assets.cross_icon} className='cursor-pointer absolute top-2 right-4 w-4' alt="" />
                </div>
              </div>
            )
          }
